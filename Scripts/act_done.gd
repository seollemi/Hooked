...
@onready var label = $RichTextLabel  # Adjust if your label has a different name
...
func _ready() -> void:
	...
	elif Global.act_2_done and not Global.act_2_seen:
		full_text = "ACT 2 DONE....."
		Global.act_2_seen = true
	elif Global.act_3_done and not Global.act_3_seen:
		full_text = "ACT 3 DONE....."
		Global.act_3_seen = true
	else:
		queue_free()  # Close whole CanvasLayer if no message needed
		return
...
func start_restore_typing():
	...
	queue_free()  # Close the entire CanvasLayer
