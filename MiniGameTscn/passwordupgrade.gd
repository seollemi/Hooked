...
func _continue_typing_sequence() -> void:
	...
	await type_text(hint_text, "You think about a car you don't like...")
	await get_tree().create_timer(2.0).timeout

	await type_text(chosen_word, "You chose: Youabian")
	await get_tree().create_timer(2.0).timeout

	await type_text(password_stats, "Characters: 8, all lowercase")
	await get_tree().create_timer(2.0).timeout

	await type_text(hack_time_estimate, "Hackable in: ~58 minutes")
	await get_tree().create_timer(2.0).timeout
...
func type_text(label: RichTextLabel, full_text: String) -> void:
	...
	# Only for hack_time_estimate: parse BBCode after typing
