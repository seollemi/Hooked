...
func _continue_typing_sequence() -> void:
	...
	await type_text(chosen_word, "You chosen the letter A: YouAbiAn...")
	await get_tree().create_timer(2.0).timeout

	await type_text(password_stats, "Characters: 8, mix of upper and lower...")
	await get_tree().create_timer(2.0).timeout

	await type_text(hack_time_estimate, "Hackable in: ~30 days")
	await get_tree().create_timer(2.0).timeout
