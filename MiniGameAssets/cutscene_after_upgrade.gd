...
func _continue_typing_sequence() -> void:
	...
	await get_tree().create_timer(2.0).timeout  # 🕑 wait 2 more seconds before changing scene
	get_tree().change_scene_to_file("res://MiniGameTscn/Hints_page.tscn")  # 🚪 Go to Hints_page
